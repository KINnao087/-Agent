...
def _build_ocr2json_user_message(config: AgentConfig, ocr_json: dict) -> str:
    """根据 OCR JSON 和配置模板构造发给模型的用户消息。"""
    ...
    file_name = Path(str(ocr_json.get("input_path", ""))).name
    ocr_json_text = json.dumps(ocr_json, ensure_ascii=False, indent=2)
